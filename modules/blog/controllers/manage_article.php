...
class Manage_Article extends CMS_Priv_Strict_Controller {
    ...
    public function video(){
        ...
        // show the view
        $this->view($this->cms_module_path().'/manage_video_view', $output,
            $this->cms_complete_navigation_name('manage_video'), $config);
    ...
    // returned on view
    public function callback_column_link($value, $row){
        $sql = $this->db->where('id', $row->id)->get($this->cms_complete_table_name('video'))->row();
        $link = base_url().'modules/blog/assets/uploads/video/'.$sql->nama;
        return '<input type="text" style="width:500px;" value="'.$link.'" readonly>';
    }

    public function callback_column_nama($value, $row){
        return '<a href="'.base_url('manage_article/show_video/'.$row->id).'">'.$value.'</a>';
    }

    public function show_video($id){
        $data = array();
        $data['sql'] = $this->db->where('id', $id)->get('cms_blog_video')->result();
